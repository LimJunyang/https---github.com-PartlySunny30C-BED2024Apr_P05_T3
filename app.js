...
app.put('/books/:book_Id/availability', authPage(["librarian"]), async (req, res) => {
    const bookId = req.params.book_Id;
    const { availability } = req.body;

    try {
        // Validate the update of availability using an external validator function
        

        // Proceed with updating the availability in the database
        const pool = await poolPromise;

        // Check if the book exists based on bookId
        const result = await pool
            .request()
            .input('book_Id', sql.Int, bookId)
            .query('SELECT TOP 1 * FROM books WHERE book_Id = @book_Id');
